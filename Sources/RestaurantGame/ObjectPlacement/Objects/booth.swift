...
extension ObjectSpawner {
    ...
    fileprivate static func spawnBoothFull(_ game: Game, _ position: Position3) -> Entity {
        ...
        let transFn = { (pos: GridPos) in
            let npos = GridPos(
                pos.x + 1,
                pos.y
            )
            return gridPosToFloorPos(npos)
        }
        
        booth_deco.configure(CustomGridPositionTransformComponent.self) { component in
            component.setTransorm(transFn)
        }
        
        cube.configure(CustomGridPositionTransformComponent.self) { component in
            component.setTransorm(transFn)
        }
